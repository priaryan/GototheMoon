"""Test EMERALDS quote width impact on Rust backtester.
Tests multiple EMERALDS quote placements while keeping TOMATOES unchanged."""
...
EMERALD_BID_OFFSET = 1  # ticks below fair
EMERALD_ASK_OFFSET = 1  # ticks above fair

class EmeraldsMM:
    FAIR = 10000
    LIMIT = POS_LIMITS["EMERALDS"]

    def generate_orders(self, depth, position, bid_offset=None, ask_offset=None):
        ...
        # MAKE: configurable offset from fair
        if bid_offset is not None:
            bid_price = fair - bid_offset
            ask_price = fair + ask_offset
        else:
            # Default: wall-mid style
